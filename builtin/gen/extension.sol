...
contract Extension {
    ...
    function blockID(uint256 num) public view returns (bytes32) {
        return ExtensionNative(this).native_blockID(uint32(num));
    }

    function blockTotalScore(uint256 num) public view returns (uint64) {
        return ExtensionNative(this).native_blockTotalScore(uint32(num));
    }

    function blockTime(uint256 num) public view returns (uint256) {
        return ExtensionNative(this).native_blockTime(uint32(num));
    }

    function blockSigner(uint256 num) public view returns (address) {
