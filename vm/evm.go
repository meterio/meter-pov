...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int, token byte) (ret []byte, leftOverGas uint64, err error) {
	...
	if !evm.Context.CanTransfer(evm.StateDB, caller.Address(), value, token) {
...
func (evm *EVM) Create2(caller ContractRef, code []byte, gas uint64, endowment *big.Int, token byte, salt *big.Int) (ret []byte, contractAddr common.Address, leftOverGas uint64, err error) {
	...
	return evm.create(caller, code, gas, endowment, token, contractAddr) // endowment is value.
}
