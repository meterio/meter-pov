...
type (
	...
	TransferFunc func(StateDB, common.Address, common.Address, *big.Int, byte)
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int, token byte) (ret []byte, leftOverGas uint64, err error) {
	...
	if value.Sign() != 0 && !evm.Context.CanTransfer(evm.StateDB, caller.Address(), value, token) {
	...
	if !evm.StateDB.Exist(addr) {
		...
		//	precompiles = PrecompiledContractsByzantium
		//}
		if !isPrecompile && evm.ChainConfig().IsEIP158(evm.BlockNumber) && value.Sign() == 0 {
	...
	evm.Transfer(evm.StateDB, caller.Address(), to.Address(), value, token)
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int, token byte) (ret []byte, leftOverGas uint64, err error) {
	...
	if !evm.CanTransfer(evm.StateDB, caller.Address(), value, token) {
		return nil, gas, ErrInsufficientBalance
	}
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *big.Int, token byte, contractAddr common.Address) ([]byte, common.Address, uint64, error) {
...
func (evm *EVM) Create(caller ContractRef, code []byte, gas uint64, value *big.Int, token byte) (ret []byte, contractAddr common.Address, leftOverGas uint64, err error) {
	...
	return evm.create(caller, &codeAndHash{code:code}, gas, value, token, contractAddr)
}
...
func (evm *EVM) Create2(caller ContractRef, code []byte, gas uint64, endowment *big.Int, token byte, salt *uint256.Int) (ret []byte, contractAddr common.Address, leftOverGas uint64, err error) {
