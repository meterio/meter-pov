...
func (s *ticketStore) addTopic(topic Topic, register bool) {
	slog.Debug("Adding discovery topic", "topic", topic, "register", register)
...
func (s *ticketStore) removeRegisterTopic(topic Topic) {
	slog.Debug("Removing discovery topic", "topic", topic)
	if s.tickets[topic] == nil {
		slog.Warn("Removing non-existent discovery topic", "topic", topic)
		return
	}
...
func (s *ticketStore) nextRegisterLookup() (lookupInfo, time.Duration) {
	...
	for len(s.regQueue) > 0 {
		...
		if s.tickets[topic].nextLookup < mclock.Now() {
			...
			slog.Debug("Found discovery topic to register", "topic", topic, "target", next.target, "delay", delay)
			return next, delay
		}
	}
	// No registration topics found or all exhausted, sleep
	delay := 40 * time.Second
	slog.Debug("No topic found to register", "delay", delay)
...
func (s *ticketStore) ticketsInWindow(topic Topic) []ticketRef {
	...
	if s.tickets[topic] == nil {
		slog.Warn("Listing non-existing discovery tickets", "topic", topic)
		return nil
	}
	...
	slog.Debug("Retrieved discovery registration tickets", "topic", topic, "from", s.lastBucketFetched, "tickets", len(tickets))
	return tickets
}
...
func (s *ticketStore) addTicketRef(r ticketRef) {
	...
	if tickets == nil {
		slog.Warn("Adding ticket to non-existent topic", "topic", topic)
		return
	}
...
func (s *ticketStore) nextFilteredTicket() (*ticketRef, time.Duration) {
	...
	for {
		...
		slog.Debug("Found discovery ticket to register", "node", ticket.t.node, "serial", ticket.t.serial, "wait", wait)
...
func (s *ticketStore) removeTicketRef(ref ticketRef) {
	slog.Debug("Removing discovery ticket reference", "node", ref.t.node.ID, "serial", ref.t.serial)
	...
	if tickets == nil {
		slog.Debug("Removing tickets from unknown topic", "topic", topic)
		return
	}
...
func (s *ticketStore) addTicket(localTime mclock.AbsTime, pingHash []byte, ticket *ticket) {
	slog.Debug("Adding discovery ticket", "node", ticket.node.ID, "serial", ticket.serial)
...
func (s *ticketStore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
		slog.Debug("Retrieving node ticket", "node", node.ID, "serial", nil)
	} else {
		slog.Debug("Retrieving node ticket", "node", node.ID, "serial", s.nodes[node].serial)
	}
	return s.nodes[node]
}
