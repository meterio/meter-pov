...
func (p *Pacemaker) precommitBlock(blkInfo *ProposedBlockInfo) error {
	...
	p.logger.Info("block precommited by consensus", "num", blk.Number(), "id", blk.ID())
	return nil
}
...
func (p *Pacemaker) commitBlock(blkInfo *ProposedBlockInfo, bestQC *block.QuorumCert) error {
	...
	p.logger.Info("block committed by consensus", "num", blk.Number(), "id", blk.ID(), "txs", len(blk.Txs))
